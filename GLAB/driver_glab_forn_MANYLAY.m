function [w,dglab] = driver_glab_lblrtm_forn_MANYLAY(gid,v1,v2,ipfile,iDoGlab,iTalk)

%% >>>>>>>>> see code in /home/sergio/HITRAN2UMBCLBL/MAKEIR/H2012/MAKEIR_ALL_H12_NOBASEMENT <<<<<<<<<<<<

%% function [w,dglab] = driver_glab_forn(gid,v1,v2,ipfile)  assumes N2 broadning, does NOT run "run8"
...
%%
%% optional input
%%    iDoGlab = N or 0 for GLAB  (N can be from 1 .. 11, so run 11 toffsets in parallel)
...
if iDoGlab > 0
  cder = ['cd /home/sergio/HITRAN2UMBCLBL/GLAB/Toff_' num2str(iDoGlab,'%02d')]; eval(cder);
  for ii = 1 : mm
    fprintf(1,'w = %8.6f GLAB layer %3i out of %3i \n',v1,ii,mm)
...
elseif iDoGlab < 0
  cder = ['cd /home/sergio/HITRAN2UMBCLBL/GLAB/Toff2_' num2str(abs(iDoGlab),'%02d')]; eval(cder);
  for ii = 1 : mm
    fprintf(1,'w = %8.6f GLAB layer %3i out of %3i \n',v1,ii,mm)
    [frglab,dglab(ii,:)]   = dogenln_onegas_sergio2(gid,v1,v2,ipfile,ii);
    %% [frglab,dglabN2(ii,:)] = dogenln_onegas_sergio2(22,v1,v2,'../LBLRTM/ipfileN2',ii);
