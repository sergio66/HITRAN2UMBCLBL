function mat2forWATER(FREQ)
%function mat2forWATER(FREQ)
% MATLAB to FORTRAN conversion of the compressed transmittance data for WATER
% this is basically the same as mat2for except that it loops 5 times towards
% the end, to save the 5 water offset profiles
...
cd /umbc/scratch/Strow/sergio/ATMOSCOMP/DATA/WaterDataBase;

[nsinga(1),nlaya(1)]=size(k_compress_w01);
[nsinga(2),nlaya(2)]=size(k_compress_r);  
[nsinga(3),nlaya(3)]=size(k_compress_w3); 
[nsinga(4),nlaya(4)]=size(k_compress_w7); 
[nsinga(5),nlaya(5)]=size(k_compress_w10);

nsing=sum(nsinga)/5; nlay=sum(nlaya)/5;

if ((nsing ~= nsinga(1))| (nlay ~= nlaya(1)))
  error('check the sizes of the five k_comp matrices ')
  end 
nlay=nlay/ntemp;
...
% Variable "ktype" will say what type of compression (1=k, 2=4th-root(k), etc)
