...
%% file will contain AB CDE.G HI  JKL   which are gasID, wavenumber, temp offset, layer
%%                   12 345.7 89  012
...
while fmin <= wn2
  ...
  for tt = Stt
    ...
    fout = [dirout '/LAYERS/std' num2str(fmin) '_' num2str(gg) '_' num2str(tt+6) '_' num2str(SLayer) '.mat'];
    if exist(fout,'file') == 0 & iYes > 0
      ...
      fip = ['IPFILES/std_gx' num2str(gg) 'x_' num2str(tt+6) 'Lay_' num2str(SLayer)];
      fid = fopen(fip,'w');
