...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% now empty out the kcomp dirs, which have the compressed database

gid = 1;
  freq_boundaries
  thedir = dir([dirout '/kcomp/*.mat']);
  if length(thedir) > 0
    fprintf(1,'emptying out KCOMP dir for gasID %3i \n',gid);
    rmer = ['!/bin/rm ' dirout '/kcomp/*.mat'];
    eval(rmer);
  end
...
gid = 2;
  ...
  thedir = dir([dirout '/kcomp/*.mat']);
  if length(thedir) > 0
    fprintf(1,'emptying out KCOMP dir for gasID 2 - 81 \n');
    rmer = ['!/bin/rm ' dirout '/kcomp/*.mat'];
    eval(rmer);
  end
