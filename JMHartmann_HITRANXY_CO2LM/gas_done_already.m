...
%% input
%%   iWhichFreqBoundaries = default (-1 for this database, or eg 2008 or 2016)
%%
%% output
...
elseif iWhichFreqBoundaries == 2016
  ...
  lenx = length('H2016_RUN8_NIRDATABASE');
  diroutX = dirout;
  diroutX(lala:lala+lenx-1) = 'H2016_RUN8_NIRDATABASE';
  dirout = diroutX;
...
lines = show_vis_ir_lines_wavenumber(2016,7,gasid);
...
if gasid > 1 
  ...
  for ff = fA : df : fB
    ...
    for kk = 1 : 11
      ...
      %%% fname = ['std' num2str(ff,'%04d') '_' num2str(gasid) '_' num2str(kk,'%02d') '.mat'];
      dirr = dir(fname);
