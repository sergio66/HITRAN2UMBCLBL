function [B,s] = absbasis_gid9(gid, gdir, vchunk, bopts)

% function [B,s] = absbasis_gid9(gid, gdir, vchunk, bopts)
%
% find a basis for compression of monochromatic absorption coeff's
% same as absbasis, but make sure it compresses larger ODs, so it can
% potentially handle Kratakoa II
%
% INPUTS
%   gid     -  gas ID
...
enhancefactor = 100;    %%%bump up ODs by this much
...
if gid == 1 | gid == 103 | gid == 110
  ...
  ksvd = reshape(k(:, slayind, stemind), 10000, ...
                    length(slayind)*length(stemind))*enhancefactor;

  % build test set
  ktrue = k(:,:,ttemind)*enhancefactor;
  clear k
end
