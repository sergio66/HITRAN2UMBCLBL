function cmprunIR_WV_LBLRTM_VERYHIGHRES(glist, vlist, HITRAN)

%% call with cmprunIR_WV_H12_VERYHIGHRES([1 103 110],605:1:855);
%% call with cmprunIR_WV_H12_VERYHIGHRES([      110],605:1:855);
...
if HITRAN == 2012
  ...
  iUsualORHigh = -2;    %%% these are high res, using 0.0001 cm-1 output
  iUsualORHigh = -3;    %%% these are high res, using 0.0002 cm-1 output
  freq_boundariesLBL    %%% these are high res, using 0.0005 or 0.0001 cm-1 output
end

cdirALL = [dirout '/kcomp.h2o/0.0001/']; 
...
cdir_1_103 = [dirout '/kcomp.h2o/0.0001/']; 
...
if HITRAN == 2012
  ...
  cdir_110 = [dirout '/kcomp.h2o/0.0001/']; 
else
  cder = ['cd /home/sergio/HITRAN2UMBCLBL/MAKEIR/H2016_G2015/MAKEIR_CO2_O3_N2O_CO_CH4_othergases_LBLRTM12p8']; eval(cder);
  iUsualORHigh = -1;    %%% these are high res, using 0.0005 cm-1 output
  iUsualORHigh = -2;    %%% these are high res, using 0.0001 cm-1 output
  iUsualORHigh = -3;    %%% these are high res, using 0.0002 cm-1 output
  freq_boundariesLBL    %%% these are high res, using 0.0001 or 0.0005 cm-1 output
  dirout = [dirout(1:end-7) 'g110.dat/'];
  ...
  elseif iUsualORHigh == -3
    cdir_110 = [dirout '/kcomp.h2o/0.0002/']; 
  else
    error('unknown iUsualORHigh')
  end
end
...
dirout_110 = dirout;
       ...
       %% cd /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//g1.dat/lblrtm0.0001/
       %% ln -s /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//lblrtm12.8/all/abs.dat0.0001// abs.dat
       %% mkdir kcomp

       %% cd /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//g1.dat/lblrtm0.0002/
       %% ln -s /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//lblrtm12.8/all/abs.dat0.0002// abs.dat
       %% mkdir kcomp

%hmm, this looks like junk ...
%ln -s /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//lblrtm12.8/WV/abs.dat0.0001/ /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830/g110.dat/lblrtm0.0001//abs.dat/
%ln -s /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830//lblrtm12.8/WV/abs.dat0.0002/ /asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830/g110.dat/lblrtm0.0002//abs.dat/
...
for gid00 = 1 : length(glist)
   ...
   switch gid0
     case 1
       ...
       cdir = [dirout_1 '/kcomp.h2o/0.0001/']; 
       gidx = gid;
     case 103
       gid = gid0;
       vlist = hdochunks;
       gdir = [dirout_103 '/abs.dat/']; 
       cdir = [dirout_103 '/kcomp.h2o/0.0001/']; 
       gidx = gid;
     case 110
       gid = gid0;
       vlist = hdochunks;
       ...
       elseif iUsualORHigh == -2
         gdir = [dirout_110 '/abs.dat/'];
         gdir = [dirout_110 '/WV/abs.dat0.0001/'];
         cdir = [dirout_110 '/kcomp.h2o/0.0001/'];
       elseif iUsualORHigh == -3
         gdir = [dirout_110 '/abs.dat/'];
         gdir = [dirout_110 '/WV/abs.dat0.0002/'];
         cdir = [dirout_110 '/kcomp.h2o/0.0002/'];
       end
       gidx = 1; 
     otherwise
       gid0
