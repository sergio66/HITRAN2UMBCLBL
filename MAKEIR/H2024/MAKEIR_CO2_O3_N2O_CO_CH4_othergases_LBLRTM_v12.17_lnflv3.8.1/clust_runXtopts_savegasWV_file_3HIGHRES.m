%% same as clust_runXtopts_savegasN_file_*.m except it figures which  "Toff3_" subdirs ....

%% this simply does all wavenumbers for gN

% now   sbatch --array=23-33 sergio_matlab_jobB.sbatch
...
addpather = ['addpath /home/sergio/HITRAN2UMBCLBL/LBLRTM/ToffWV3_' num2str(Stoffset,'%02d')]; eval(addpather);
...
while fmin <= wn2
  ...
  for tt = Stt
    ...
    if exist(fout,'file') == 0 & iYes > 0
      ...
      cder = ['cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/ToffWV3_' num2str(Stoffset,'%02d')]; eval(cder);
