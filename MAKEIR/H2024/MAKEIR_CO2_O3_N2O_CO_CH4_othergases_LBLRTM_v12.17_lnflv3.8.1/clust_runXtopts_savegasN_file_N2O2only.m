%% same as clust_runXtopts_savegasN_file_2.m except it goes to "Toff_" subdirs .... and only makes O2 or N2
%% better to "run" for gid == 7 (since gid for N2 - 22 and is greater than 12, was too lazy to 
%% code driver_glab_lblrtm_forn_MANYLAY_N2O2true and driver_glab_lblrtm_forn_MANYLAY_N2O2fake for gas id > 12

%% this simply does all wavenumbers for gN

% local running to test
% clustcmd -L clust_runXtopts_savegasN_file_N2O2only.m '070173001'
%
% otherwise when happy
% clustcmd -q medium -n 11 clust_runXtopts_savegasN_file_N2O2only.m file_parallelprocess_N2O2.txt
%
% or
% clustcmd -q long_contrib -n 11 clust_runXtopts_savegasN_file_N2O2only.m file_parallelprocess_N2O2.txt

%% file will contain AB CDEFG HI  which are gasID, wavenumber, temp offset   
%%                   12 34567 89
%% where gasID = 01 .. 99,   HI = 1 .. 11 (for Toff = -5 : +5) and wavenumber = 00050:99999

%% test eg JOB='070223006'; clust_runXtopts_savegasN_file_N2O2only.m
...
adderpath
addpather = ['addpath /home/sergio/HITRAN2UMBCLBL/LBLRTM/ToffALL/Toff_' num2str(Stoffset,'%02d')]; eval(addpather);
...
while fmin <= wn2
  ...
  for tt = Stt
    ...
    if exist(fout,'file') == 0 & iYes > 0
      ...
      cd /home/sergio/SPECTRA
      if gid == 7 | gid == 22
        profile = [(1:100)' refpro.mpres refpro.gpart(:,gg7) tprof refpro.gamnt(:,gg7)]';
      end
      ...
      cder = ['cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/ToffALL/Toff_' num2str(Stoffset,'%02d')]; eval(cder);

      %[w,dglab,dlblrtm] = driver_glab_lblrtm_forn_MANYLAY(gasid,fmin,fmax,['/home/sergio/SPECTRA/' fip],-1,-1);
      %dall = dlblrtm;

      %% coompute ODs due ONLY to (O2+N2) 
      [w,dglab,dlblrtm] = driver_glab_lblrtm_forn_MANYLAY_N2O2true(gasid,fmin,fmax,['/home/sergio/SPECTRA/' fip],-1,-1);
      dN2O2 = dlblrtm;
