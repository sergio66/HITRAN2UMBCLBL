...
% now   sbatch --array=45-55 sergio_matlab_jobB.sbatch
...
adderpath
addpather = ['addpath ' outputdirToffALL '/ToffWV5_' num2str(Stoffset,'%02d')]; eval(addpather);
...
while fmin <= wn2
  ...
  for tt = Stt
    ...
    if exist(fout,'file') == 0 & iYes > 0
      ...
      cder = ['cd ' outputdirToffALL '/ToffWV5_' num2str(Stoffset,'%02d')]; eval(cder);
