...
if dirout(length(dirout)) == '/'
  dirout = dirout(1:length(dirout)-1);
  end
...
while fmin <= wn2
  ...
  k = zeros(10000,100);
  iSave = 0;
  fout = [dirX  '/std' num2str(fmin) '_2_6voigtHARTMANN.mat'];
  ee = exist(fout);
  if ee ~= 0
    eeX = +1;
    fprintf(1,'lblumbc kdata files exist, but %s already made \n',fout);
    end
  for pp = 0 : 0         %% loop over temperature
    ...
    if eeX == -1 & pp == 0 & eeG > 0 & eeV > 0
      figure(3); pcolor(fr,1:100,dV./k'); shading interp; colorbar 
      title('V/H'); 
      end
    end                 %% loop over temperature (1..11) pp

  if sum(kid(:)) ~= 100 & eeX == -1
