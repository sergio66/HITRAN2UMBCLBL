...
% clustcmd -L g2_forwards_runXtopts_savegasX_LTE.m 1:11
%
% otherwise when happy
% clustcmd -q medium -n 11 g2_forwards_runXtopts_savegasX_LTE.m 1:11
...
fprintf(1,'this is g2_forwards_runXtopts_savegasX_LTE.m solzen = %3i TOFF = %3i \n',solzen,iProfile*10);
...
while fmin <= wn2
  ...
  for pp = iProfile
    ...
    dirout = [dirout0 '/' num2str(solzen) '_LTE/'];
    ...
    if gasid > 1 & ee == 0
      ...
      make_fstd_ip(dirout0,solzen,ppp);
      ...
      sedder = [sedder homedir '/quickuseLTE_template.nml > ' fnml];
      eval(sedder);
