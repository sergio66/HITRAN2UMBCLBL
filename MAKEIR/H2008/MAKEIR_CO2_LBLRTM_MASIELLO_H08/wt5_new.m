...
function wt5_new(title,hbound,vbound,tbound,pbound,altitude,pressure,temperature,nmolinp,idmolinp,unitinp,wmol,aflag,CN,cnt_mul)
...
% hmol_scal    NOT AVAILABLE scaling type (' ' = No scaling, '0' = scaling factor is 0, '1' scaling applied directly to profile, )
...
%
% function wt5_new(title,hbound,vbound,tbound,pbound,altitude,pressure,temperature,nmolinp,idmolinp,unitinp,wmol,mol_scal,hmol_scal,xmol_scal,aflag,CN,cnt_mul)
% function wt5_new(title,hbound,vbound,tbound,pbound,altitude,pressure,temperature,nmolinp,idmolinp,unitinp,wmol,aflag,CN,cnt_mul)
%
...
CO2MX = 330; 	                       % Reference CO2 mixing ratio (ppmv)
REF_LAT=0;
altxs1=max(pressure);
altxs2=min(pressure);
nlayertot=no_input_levels;
if min(pressure)>hbound(1)
    aa1=(floor(max(altitude)/10)+1)*10.;
    aa2=90.;
    alt=[aa1:10:aa2];
    altxs2=aa2;
...
for i = 1:no_input_levels
    fprintf(fid,['%10.3f%10.3f%10.3f     AA   ' form '\n'],[altitude(i) pressure(i) temperature(i)]);
    str1=str0;
    for m=1:nmolinp
        dum=wmol(i,m);
        if dum<0
