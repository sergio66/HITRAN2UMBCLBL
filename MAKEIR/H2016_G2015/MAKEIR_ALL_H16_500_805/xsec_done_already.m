...
gid = gasid;
freq_boundaries
fA = wn1; fB = wn2; df = dv; df = 25; df = 5;
dir0 = dirout;
...
for wn = wn1 : dv : wn2
  ...
  woo = findxsec_plot_fast(wn-df25,wn+dv+df25,bands);
  if woo >= 1
    iCnt = iCnt + 1;
    iaChunk(iCnt) = wn;
...
if nn > 0
  ...
  oo = find(lines.wnum >= fA-df25 &  lines.wnum <= fB+df25);
  if length(oo) > 0

    figure(1)
    plot(lines.wnum(oo),lines.stren(oo),'+');
    axis([fA-df fB+df min(lines.stren(oo))*0.999999999999999  max(lines.stren(oo))*1.0000000000001])
    %axis([max(fA-df,min(bands.v1)-df) min(fB+df,max(bands.v2)+df) min(lines.stren(oo))*0.999999999999999  max(lines.stren(oo))*1.000000001])
    ax = axis;
...
%      yesno(mm) = 0;
  ...
  if sum(abs(yesno(bwop))) > 0
    disp(' >>>>>>>>>> WARNING : somehow found different chunk files made by run8, than what we expect from findlines_plot');
  end
...
if jjx > 0
  iDone = 0;
  for ii = 1 : iCnt
    woo = find(freqchunk == iaChunk(ii),1);
    if length(woo) == 1
