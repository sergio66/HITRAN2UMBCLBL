...
disp('iDir = LM executable : (1) Orig (2) only fixed rdmult (3) fixed rdmult and Qtips and MassIso (4) extend ru8 to 2500 cm-1');
...
origname = ['/asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830/g2.dat_run8_H2016params_400ppm_tsp0_selfbroadTfix/sampleT_'];
if iDir == 1
...
elseif iDir == 4
  finalname = ['/asl/s1/sergio/H2016_RUN8_NIRDATABASE/IR_605_2830/g2.dat_run8_H2016params_400ppm_tsp0_selfbroadTfix_rdmult_newQtip1250/sampleT_'];
end

origname  = [origname num2str(iChunk) '_2_6.mat'];
finalname = [finalname num2str(iChunk) '_2_6.mat'];
lslter = ['!ls -lt ' origname];  eval(lslter);
lslter = ['!ls -lt ' finalname]; eval(lslter);

load /home/sergio/HITRAN2UMBCLBL/REFPROF/refproTRUE.mat         %% symbolic link

if exist(finalname) & exist(origname)
  loader = ['a0 = load(''' origname ''');'];
  loader = ['load ' origname];
  eval(loader)
  ...
  d0 = d0(1:mm,:);
  dvoigt0 = dvoigt0(1:mm,:);
  dfirst0 = dfirst0(1:mm,:);
  dfull0 = dfull0(1:mm,:);  
  ...
  figure(1); plot(w,dvoigt0./d0,'b',w,dvoigt./d,'r',w,d0./d,'k'); title('LM/UMBC Voigt (b) orig (r) new (k)me0/me');
  figure(2); semilogy(w,d(1,:),'b',w+1.5*0.0025,dvoigt(1,:),'r',w,d0(1,:),'c',w+1.5*0.0025,dvoigt0(1,:),'m');
    hl = legend('UMBC new','LM Voigt new','UMBC old','LM Voigt old');
  ...
  figure(3); plot(T,dvoigt(:,ix)./d(:,ix),'bo-',T,dvoigt(:,iy)./d(:,iy),'rx-',T,dvoigt0(:,ix)./d0(:,ix),'cd-',T,dvoigt0(:,iy)./d0(:,iy),'mo-');
  %hl = legend(num2str((T)'));
  hl = legend('minOD new','maxOD new','minOD old','maxOD old');
  xlabel('T'); ylabel('LMvoigt/UMBCvoigt');
