function [data] = lbl_read_dbl(filename,endian,pflag);

%
% function [data] = lbl_read_dbl_le(filename,endian,pflag);
%
% Read in an LBLRTM TAPE12 double precision file (which is an f90 unformatted sequential
...
%         pflag - status flag: 1 to show status while reading, 0 to
%         not show status.
...
data.file_header.user_id = char(fread(fid,80,'uchar'))';
data.file_header.secant = fread(fid,1,'float64');
data.file_header.p_ave = fread(fid,1,'float64');
data.file_header.t_ave = fread(fid,1,'float64');
molecule_id = zeros(64,8);
for i = 1:64;
    molecule_id(i,:)= fread(fid,8,'uchar')';
end
data.file_header.molecule_id = char(molecule_id);
data.file_header.mol_col_dens = fread(fid,64,'float64');
data.file_header.broad_dens = fread(fid,1,'float64');
data.file_header.dv = fread(fid,1,'float64');
data.file_header.v1 = fread(fid,1,'float64');
data.file_header.v2 = fread(fid,1,'float64');
%GM
v1=data.file_header.v1;
v2=data.file_header.v2;
%GM
data.file_header.t_bound = fread(fid,1,'float64');
data.file_header.emis_bound = fread(fid,1,'float64');
...
if endian=='l';    gm = fread(fid,1,'int');end
data.file_header.LBL_id.scnid = fread(fid,1,'float64');
data.file_header.LBL_id.hwhm = fread(fid,1,'float64');
if endian=='b';    gm = fread(fid,1,'int');end
...
data.file_header.yi1 = fread(fid,1,'float64');
yid = zeros(10,8);
...
while end_of_file == 0
  ...
  % read in 4 bytes before and after every record 
   ...
   data.panel_header.dv{data.n_panels+1} = fread(fid,1,'float64');
   if endian=='b';    gm = fread(fid,1,'int');  end
  ...
  if data.panel_header.n_pts{data.n_panels+1} ~= -99
	switch upper(data.file_type)
		...
		junk = fread(fid,4,'char');
		rad = fread(fid,data.panel_header.n_pts{data.n_panels+1},'float64');
		junk = fread(fid,4,'char');
		junk = fread(fid,4,'char');
		tau = fread(fid,data.panel_header.n_pts{data.n_panels+1},'float64');
		junk = fread(fid,4,'char');
