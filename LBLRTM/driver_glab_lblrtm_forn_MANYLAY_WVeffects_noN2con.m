...
%% same as driver_glab_lblrtm_forn_MANYLAY_noN2con.m but also includes US Std WV broadening
...
%% function [w,dglab,dlblrtm] = driver_glab_lblrtm_forn(gid,v1,v2,ipfile,ipfilewv)  assumes N2 broadning, does NOT run "run8"
...
%%   ipfilewv = one layer of profile info [index   p(atm)     pp(atm)    T(K)  q (kmol/cm2)]
%%   
%% optional input
...
%%
%% example 
%%[w,dglab,dlblrtm] = driver_glab_lblrtm_forn_MANYLAY_WVeffects(2,2130,2180,'/home/sergio/SPECTRA/IPFILES/co2two','/home/sergio/SPECTRA/IPFILES/wvtwo');
...
if nargin < 5
  error('need gas profile and wv profile')
end
...
if iDoGlab > -2
  for ii = iiStart : iiEnd
    ...
    if gid == 2
      if iType == 605
        qfactor = makeTAPE5_noN2con_WVeffects(gid,v1-25,v2+25,ipfile,ipfilewv,dvx,ii,iTalk); %% Mlawer suggested larger bracket than 1 cm-1
      elseif iType == 500
        qfactor = makeTAPE5_noN2con_WVeffects(gid,v1-15,v2+15,ipfile,ipfilewv,dvx,ii,iTalk); %% Mlawer suggested larger bracket than 1 cm-1
      end
    elseif gid <= 47 
      if iType == 605
