function [w,wglab,drun8,dglab,dlblrtm] = driver_glab_lblrtm_forn_ONELAY(gid,v1,v2,ipfile0,N)

%% function [w,drun8,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(gid,v1,v2,ipfile)  assumes N2 broadning
%% assumes 0.0025 cm-1 output resolution
%% input
%%   gid    = gasID
%%   v1,v2  = start/stop wavenumber
%%   ipfile = one layer of profile info [index   p(atm)     pp(atm)    T(K)  q (kmol/cm2)]
%%   N : default is ONE since it assumes ipfile is single layer  
%% output
%%   dglab           = ODs from GLAB/GENLN2
%%   dlblrtm         = ODs from LBLRTM
%%
%% the 5 column ipfile information is
%%   layer no (integer)      total press (atm)    partial press (atm)      Temp (K)      gas amount (kilomoles/cm2)
%%
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_self(5,2080,2130,'/home/sergio/SPECTRA/IPFILES/co_gascell1_self');
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(5,2080,2130,'/home/sergio/SPECTRA/IPFILES/co_gascell1');
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(3,1030,1055,'/home/sergio/SPECTRA/IPFILES/ozone_97'); BAD for GENLN2
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(3,1030,1055,'/home/sergio/SPECTRA/IPFILES/co_gascell1');
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_self(3,1030,1055,'/home/sergio/SPECTRA/IPFILES/co_gascell1_self');
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(2,730,755,'/home/sergio/SPECTRA/IPFILES/co2one');
%%
%% example [w,drun8,drun8noshift,drun8shiftneg1,dglab,dlblrtm] = driver_run8_glab_lblrtm_forn(2,730,755,'/home/sergio/SPECTRA/IPFILES/co2_100profile',56);
  
if v1 < 600 | v2 > 3000
  error('TAPE5 is set up for dv=0.0005 cm-1 so only use this code for 607-2830 cm-1')
end

%cd /home/sergio/HITRAN2UMBCLBL/GLAB
%[wglab,dglab] = dogenln_onegas_sergio2(gid,v1,v2,ipfile);
%cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/
%plot(wglab,dglab)
%error('ooo')

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% run8

ipfile = ipfile0;
wah = load(ipfile,'-ascii');
[mm,nn] = size(wah);
if mm > 1
  fprintf(1,'orig profile has size [mm nn] = %4i x %2i ---> subbing it to row %3i \n',mm,nn,N)
  fname = mktempS('glab_lblrtm_forn_ONELAY.txt');
  fid = fopen(fname,'w');
  wah = wah(N,:);
  fprintf(fid,'%3i %8.4e %8.4e %8.4f %12.6e \n',wah);
  fclose(fid);
  ipfile = fname;
end

fprintf(1,'driver_glab_lblrtm_forn_ONELAY.m : input profile = %s \n',ipfile);
catter = ['!cat ' ipfile]; eval(catter);
disp('catted the cat in driver_glab_lblrtm_forn_ONELAY.m')

%% see /umbc/rs/pi_sergio/WorkDirDec2025/HITRAN2UMBCLBL/MAKEIR/H2024/MAKEIR_CO2_O3_N2O_CO_CH4_othergases_LBLRTM_v12.17_lnflv3.8.1/load_refprof.m
% gah = load('/home/sergio/HITRAN2UMBCLBL/REFPROF/refproTRUE.mat');
% moo = [N gah.refpro.mpres(N) gah.refpro.gpart(N,gid) gah.refpro.mtemp(N) gah.refpro.gamnt(N,gid)];
% fprintf(1,'moo at driver_glab_lblrtm_forn_ONELAY.m = %3i %8.4e %8.4e %8.5f %8.4e \n',moo);
% fprintf(1,'T_refprof - Tactualprof = %8.4f \n',moo(4)-wah(4))

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

addpath /home/sergio/SPECTRA
if gid > 2 & gid < 51
  [w,drun8] = run8(gid,v1,v2,ipfile);
  dnoshift = drun8;
  dshiftneg1 = drun8;

  iShift = -1;
  if iShift < 0
    disp('for speed purposes, not doing dnoshift and dshiftneg1 ... just setting them equal to d!!!')
    disp('can set iShift = +1 if you wish them to be computed')
  else
    topt.tsp_mult = 0;  
    [w,dnoshift] = run8(gid,v1,v2,ipfile,topt);

    topt.tsp_mult = -1;
    topt.LVG = 'W';
    % topt.HITRAN = '/asl/data/hitran/h08.by.gas';
    [w,dshiftneg1] = run8(gid,v1,v2,ipfile,topt);
  end
  
elseif gid == 2
  %[w,d] = run8co2(gid,v1,v2,ipfile);
  disp('very quickly doing run8 instead of run8co2')
  [w,drun8] = run8(gid,v1,v2,ipfile);  

  dnoshift = drun8;
  dshiftneg1 = drun8;

  iShift = -1;
  if iShift < 0
    disp('for speed purposes, not doing dnoshift and dshiftneg1 ... just setting them equal to d!!!')
    disp('can set iShift = +1 if you wish them to be computed')
  else
    % topt.tsp_mult = 0;
    % [w,dnoshift] = run8co2(gid,v1,v2,ipfile,topt);
    dnoshift   = drun8;
    dshiftneg1 = drun8;
  end
  
elseif gid == 1
  [w,drun8]    = run8water(gid,v1,v2,ipfile);
  topt.CKD = 25;
  [w,dcon] = run8watercontinuum(gid,v1,v2,ipfile,topt);
  drun8 = drun8 + dcon;

  dnoshift = drun8;
  dshiftneg1 = drun8;
  
else
  fprintf(1,'gid = %3i \n',gid);
  error('gid not supported!')
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

iAddN2 = +1;
iAddN2 = -1;
%% now add N2
if iAddN2 > 0
  gas = load(ipfile);
  L = gas(5)*8.31446218*gas(4)/(gas(3)*101325)*1000*10000*100;   %% pV = nRT ==> q = pL/RT  and then units

  mr = gas(3)/gas(2);
  n2 = gas(5)*(1-mr)/mr;

  gas(3) = gas(2) - gas(3);
  gas(5) = n2;
  fid = fopen('ipfileN2','w');
  fprintf(fid,'%3i %8.6e %8.6e %8.6f %8.6e \n',gas);
  fclose(fid);
  [w,dN2] = run8(22,v1,v2,'../LBLRTM/ipfileN2');
else
  dN2 = zeros(size(drun8));
end

drun8      = drun8      + dN2;
dnoshift   = dnoshift   + dN2;
dshiftneg1 = dshiftneg1 + dN2;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% LBLRTM

cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/
addpath XHUANG
dvx = 0.0025;
dvx = 0.0005;

cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/ToffALL/Toff_01

iTalk   = +1;       %% assume talk during makeTAPE5
% qfactor = makeTAPE5_noN2con(gid,v1-01,v2+01,ipfile,dvx,1,iTalk); %% Mlawer suggested larger bracket than 1 cm-1
% qfactor = makeTAPE5_noN2con(gid,v1-10,v2+10,ipfile,dvx,1,iTalk); %% Mlawer suggested larger bracket than 1 cm-1
qfactor = makeTAPE5_noN2con(gid,v1-25,v2+25,ipfile,dvx,1,iTalk);   %% Mlawer suggested larger bracket than 1 cm-1

lbler = ['!lblrtm >& ugh']; eval(lbler)
catter = ['!more ugh'];     eval(catter)

lser = ['!ls -lt ODint_001'];
eval(lser)
[vx, OD, v] = lblrtm_tape11_reader_ODint('ODint_001','d');   %% lblrtm compiled with double

if dvx == 0.0005
  woop = find(vx >= v1 - 2.001*0.0005 & vx <= v2 - 0.0025 + 2.001*0.0005);
  vxx = boxint(vx(woop),5);
  ODD = boxint(OD(woop),5);
elseif dvx == 0.0025
  woop = find(vx >= v1 - 0.001 & vx <= v2 - 0.0025 + 0.001);
  vxx = vx(woop)';
  ODD = OD(woop)';
end

if length(vxx) ~= length(w)
  [length(vxx)  length(w)]
  error('length(vxx) ~= length(w)')
end

if sum(abs(vxx-w))/length(w) > 0.0001
  error('hmm size of arrays matches, but vxx, w seem different')
end

dlblrtm = interp1(vxx,ODD,w);
dlblrtm = ODD;

plot(w,drun8,vxx,ODD,'r'); legend('run8','lblrtm','location','best')			       
%plot(w,drun8 - dlblrtm,'r')

% disp('pausing here in driver_glab_lblrtm_forn_ONELAY.m')
% keyboard_nowindow

cder_here


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% GLAB/GENLN2

cd /home/sergio/HITRAN2UMBCLBL/GLAB
[wglab,dglab]   = dogenln_onegas_sergio2(gid,v1,v2,ipfile);
[wglab,dglabN2] = dogenln_onegas_sergio2(22,v1,v2,'../LBLRTM/ipfileN2');
cd /home/sergio/HITRAN2UMBCLBL/LBLRTM/

gas = load(ipfile);
L = gas(5)*8.31446218*gas(4)/(gas(3)*101325)*1000*10000*100;   %% pV = nRT ==> q = pL/RT  and then units
fprintf(1,'path cell length = %8.6f cm \n',L)
dglab = (dglab + dglabN2) * L;

semilogy(w,drun8,wglab,dglab)
dglab = dglab;

return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%{

%figure(1); semilogy(w,dshiftneg1,w,dlblrtm,'g',w,dglab,'r',vx,OD,'m','linewidth',2); grid
%  ax = axis; axis([v1 v2 ax(3) ax(4)]);
%  hl=legend('dshiftneg1','dlblrtm','dglab','ODlblrtm high res');
%  set(hl,'fontsize',10)
figure(1); semilogy(w,dshiftneg1,w,dlblrtm,'g',w,dglab,'r',w,d,'m','linewidth',2); grid
  ax = axis; axis([v1 v2 ax(3) ax(4)]);
  hl=legend('dshiftneg1','dlblrtm','dglab','d');
  set(hl,'fontsize',10)

figure(2); plot(w,dnoshift./dglab,'k',w,d./dlblrtm,'g.-',w,dshiftneg1./dlblrtm,'r','linewidth',2); grid
  ax = axis; axis([v1 v2 ax(3) ax(4)]);
  hl=legend('dnoshift/dglab','d/dlblrtm','dnew/dlblrtm');
  set(hl,'fontsize',10)

figure(3); plot(w,dnoshift-dglab,'k',w,d-dlblrtm,'g.-',w,dshiftneg1-dlblrtm,'r','linewidth',2); grid
  ax = axis; axis([v1 v2 ax(3) ax(4)]);
  hl=legend('dnoshift-dglab','d-dlblrtm','dnew-dlblrtm');
  set(hl,'fontsize',10)
%}

%{
fid = fopen('aer.txt','w');
data = [w; d; dnoshift; dshiftneg1; dglab; dlblrtm];
fprintf(fid,'%10.8e %10.8e %10.8e %10.8e %10.8e %10.8e\n',data);
fclose(fid);
%}

return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%{
keyboard

figure(1); clf
plot(w,d,vxx,ODD,'r',wglab,dglab,'k'); hl=legend('run8','lblrtm','genln'); grid

figure(2); clf
plot(w,d./d,w,d./ODD,'r',w,d./dglab,'k','linewidth',2); 
  hl=legend('run8','lblrtm','genln'); grid
  axis([v1 v2 0.8 1.2])
  axis([v1 v2 0.9 1.1])
title('dCorrect/other')

figure(3); clf
plot(w,dnoshift./d,w,dnoshift./ODD,'r',w,dnoshift./dglab,'k','linewidth',2); 
  hl=legend('run8','lblrtm','genln'); grid
  axis([v1 v2 0.8 1.2])
  axis([v1 v2 0.9 1.1])
title('dNoPshift/other')

%}
